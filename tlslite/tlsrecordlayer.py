...
class TLSRecordLayer(object):
    ...
    def __init__(self, sock):
        ...
        # heartbeat requests
        self.heartbeatCanReceive = False
        ...
        self.heartbeatSupported = False

        self.heartbeatRequestPayloads = []
                    ...
                    if recordHeader.type == ContentType.heartbeat and \
                        ...
                        try:
                            ...
                            if heartbeat_message.message_type == \
                                ...
                                if not self.heartbeatCanReceive:
                            ...
                            # request we sent
                                ...
                                heartbeat_message.checkResponse(
    ...
    def sendHeartbeatRequest(self, payload, padding_length):
        """Sends heartbeat_request and saves payload from this message for
           future checks of heartbeat responses.
        ...
        @raise socket.error: If a socket error occurs.
        """
        self.heartbeatRequestPayloads.append(payload)
        for _ in self.writeHeartbeat(payload, padding_length):
            pass

    def checkHeartbeatResponse(self, payload):
        """Checks payload from heartbeat response. If its adequate to any
           payload we have sent, then we delete this payload from list.

        @type payload: bytes
        @param payload: Payload from heartbeat response.
        """
        for heartbeat_payload in self.heartbeatRequestPayloads:
            if payload == heartbeat_payload:
                self.heartbeatRequestPayloads.remove(heartbeat_payload)
