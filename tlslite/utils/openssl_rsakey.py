...
if m2cryptoLoaded:
    ...
    class OpenSSL_RSAKey(RSAKey):
        ...
        def _raw_private_key_op_bytes(self, message):
            try:
                return bytearray(m2.rsa_private_encrypt(
                    self.rsa, bytes(message), m2.no_padding))
            except M2Crypto.RSA.RSAError:
                raise ValueError("Bad parameters to private key operation")
        ...
        def _raw_public_key_op_bytes(self, ciphertext):
            try:
                return bytearray(m2.rsa_public_decrypt(
                    self.rsa, bytes(ciphertext), m2.no_padding))
            except M2Crypto.RSA.RSAError:
                raise ValueError("Bad parameters to public key operation")
