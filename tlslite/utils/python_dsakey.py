from ecdsa.der import encode_sequence, encode_integer, read_number,  \
remove_sequence, remove_octet_string, remove_integer, remove_constructed, remove_object
...
class Python_DSAKey(DSAKey):
    ...
    def hashAndVerify(self, signature, data, hAlg="sha1"):
        ...
        if 0 < r < self.q and 0 < s < self.q:
            ...
            u1 = (digest * w) % self.q
            u2 = (r * w) % self.q
            v = ((powMod(self.g, u1, self.p) * powMod(self.public_key, u2, self.p)) % mpz(self.p)) % mpz(self.q)
            return r == v
