from .dsakey import DSAKey
from .cryptomath import *

class Python_DSAKey(DSAKey):

    def __init__(self, p=0, q=0, g=0, x=0, y=0):
        # TODO: add asserts
        self.p = p
        self.q = q
        self.g = g
        self.private_key = x
        self.public_key = y
        self.key_type = "dsa"

    def __len__(self):
        return numBits(self.p)

    def hasPrivateKey(self):
        return bool(self.private_key)

    @staticmethod
    def generate(L, N):
        # TODO: add asserts, docstrings
        key = Python_DSAKey()
        (q, p) = Python_DSAKey.generate_qp(L, N)

        index = getRandomNumber(1, (p-1))
        g = powMod(index, int((p-1)/q), p)
        x = getRandomNumber(1, q-1)
        y = powMod(g, x, p)
        key.q = q
        key.p = p
        key.g = g
        key.private_key = x
        key.public_key = y
        return key

    @staticmethod
    def generate_qp(L, N):
        # TODO: optimize, docstring
        q = int(getRandomPrime(N))
        while True:
            p = int(getRandomPrime(L))
            if (p-1) % q:
                break
        return (q, p)

    def hashAndSign(self, data, hAlg="sha1"):
        # TODO: add assert for hash size < (q-1) constrain, docstring
        hashBytes = secureHash(bytearray(data), hAlg)
        k = getRandomNumber(1, (self.q-1))
        r = int(powMod(self.g, k, self.p) % self.q)
        s = invMod(k, self.q) * (bytesToNumber(hashBytes) + self.private_key * r)
        return numberToByteArray(r) + numberToByteArray(s)
    def verify(self, signature, hash_bytes):
        raise NotImplementedError()
