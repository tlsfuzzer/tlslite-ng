...
class AESCCM(object):
    ...
    def __init__(self, key, implementation, rawAesEncrypt, tag_length=16):
        ...
        self._ctr = python_aes.new(self.key, 6)
    ...
    def seal(self, nonce, msg, aad):
        ...
        enc_msg = self._ctr.encrypt(msg, s_n)

        mac = self._cbcmac_calc(nonce, aad, msg)

        auth_value = self._ctr.encrypt(mac, s_0)
    ...
    def open(self, nonce, ciphertext, aad):
        ...
        msg = self._ctr.decrypt(ciphertext, s_n)
        ...
        received_mac = self._ctr.decrypt(auth_value, s_0)
