...
class TLSConnection(TLSRecordLayer):
    ...
    def _clientSendClientHello(self, settings, session, srpUsername,
        ...
        if (
            ...
            algos_numbers = [getattr(CompressionAlgorithm, algo) for algo
    ...
    def _clientTLS13Handshake(self, settings, session, clientHello,
        ...
        if not sr_psk:
            ...
            if isinstance(result, CompressedCertificate):
                self.server_cert_compression_algo = CompressionAlgorithm.toStr(
        ...
        if certificate_request:
            ...
            chosen_compression_algo = choose_compression_send_algo(
                self.version, certificate_request.compressed_certificate_algs,
                settings.certificate_compression_send)

            if chosen_compression_algo:
                self.client_cert_compression_algo = CompressionAlgorithm.toStr(
                    chosen_compression_algo)
                client_certificate = CompressedCertificate(
                    serverHello.certificate_type, self.version)
                client_certificate.create(
                    chosen_compression_algo, clientCertChain)
            else:
    ...
    def _clientKeyExchange(self, settings, cipherSuite,
        ...
        if certificateRequest:
            ...
            chosen_compression_algo = choose_compression_send_algo(
                self.version, certificateRequest.compressed_certificate_algs,
                settings.certificate_compression_send)

            if chosen_compression_algo:
                self.client_cert_compression_algo = CompressionAlgorithm.toStr(
                    chosen_compression_algo)
                clientCertificate = CompressedCertificate(certificateType)
            else:
    ...
    def request_post_handshake_auth(self, settings=None):
        ...
        if self.version >= (3, 4):
            if settings:
                algos_numbers = [
                    getattr(CompressionAlgorithm, algo) for algo
                    in settings.certificate_compression_receive
                ]
            else:
                algos_numbers = [1]
    ...
    def _serverTLS13Handshake(self, settings, clientHello, cipherSuite,
        ...
        if selected_psk is None:
            ...
            chosen_compression_algo = choose_compression_send_algo(
                self.version, clientHello.compressed_certificate_algs,
                settings.certificate_compression_send)
    ...
    def _serverSRPKeyExchange(self, clientHello, serverHello, verifierDB,
        ...
        if cipherSuite in CipherSuite.srpCertSuites:
            ...
            if chosen_compression_algo:
                self.server_cert_compression_algo = CompressionAlgorithm.toStr(
                    chosen_compression_algo)
                certificateMsg = CompressedCertificate(CertificateType.x509)
                certificateMsg.create(chosen_compression_algo, serverCertChain)
            else:
