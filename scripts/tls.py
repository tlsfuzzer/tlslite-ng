...
def printUsage(s=None):
  ...
  --dc-file DCFILE - the file from which to load the delegated credential
  --dc-out DCFILE - the file to store the delegated credential
...
def handleArgs(argv, argString, flagsList=[], expect_address=True):
    ...
    if "dc-out=" in flagsList:
        retList.append(dc_out)
    if "dc-file=" in flagsList:
        retList.append(delegated_credential)
    return retList
...
def serverCmd(argv):
    (address, privateKey, cert_chain, virtual_hosts, tacks, verifierDB,
            ...
            groups, dc_key, dc_pub, dc_sig_scheme, delegated_credential) = \
    ...
    if cert_chain and not privateKey:
        if not delegated_credential or not dc_key:
    ...
    if (dc_key and not delegated_credential and not dc_pub):
...
def credential_cmd(argv):
    ...
    (private_key, cert_chain, virtual_hosts, dc_pub,
            dc_sig_scheme, dc_out) = \
        handleArgs(argv, "kc", ["dc-pub=", "dc-sig-scheme=", "dc-out="],
...
def _create_delegated_credential_object(
    ...
    if cert_type == "Ed25519" or cert_type == "Ed448":
        ...
        if "BRAINPOOL" in dc_pub.curve_name:
