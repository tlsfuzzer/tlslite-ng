...
from tlslite.constants import ContentType, HandshakeType, CipherSuite, \
        CertificateType
from tlslite.mathtls import PRF_1_2, calcKey
...
class TestTLSRecordLayer(unittest.TestCase):
    ...
    def test_full_connection_with_RSA_kex(self):
        ...
        master_secret = calcKey((3, 3), premasterSecret,
                                CipherSuite.TLS_RSA_WITH_AES_128_CBC_SHA,
                                "master secret",
                                clientRandom=client_hello.random,
                                serverRandom=server_hello.random,
                                outputLength=48)
        ...
        srv_master_secret = calcKey(srv_record_layer.version,
                                    srv_premaster_secret,
                                    CipherSuite.TLS_RSA_WITH_AES_128_CBC_SHA,
                                    "master secret",
                                    clientRandom=srv_client_hello.random,
                                    serverRandom=srv_server_hello.random,
                                    outputLength=48)
