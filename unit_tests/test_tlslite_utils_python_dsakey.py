...
class TestDSAKey(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Signatures generate via OpenSSL
        # dsa sha1 signature of "some message to sign"
        cls.sha1_sig = \
                bytearray(
                        b'\x30\x2C\x02\x14\x54\x07\x13\xC9\xE6\xB4\x48\x75\x19\x4D\x88\x61'
        ...
        cls.sha1_sig_diff = \
                bytearray(
                        b'\x30\x2D\x02\x15\x00\x88\xE8\xAF\x9C\xDA\x6D\x0B\x4A\xC4\x0E\x52'
    ...
    def test_sign_verify_malformed_keys(self):
        msg = b'some message to sign'
    ...
    def test_sign_verify_diff_key(self):
        msg = b'some message to sign'

        self.assertFalse(self.key_diff.hashAndVerify(self.sha1_sig, msg))

    def test_sign_verify_diff_sign(self):
        msg = b'some message to sign'

        self.assertFalse(self.key.hashAndVerify(self.sha1_sig_diff, msg))
