...
from tlslite.constants import CipherSuite, CertificateType, ContentType, \
        ...
        KeyUpdateMessageType, CompressionAlgorithm
...
class TestCompressedCertificate(unittest.TestCase):
    ...
    def test___repr__(self):
        cc = CompressedCertificate(CertificateType.x509)
        cc = cc.create(CompressionAlgorithm.zlib,
    ...
    def test_write_empty(self):
        ...
        algos = [CompressionAlgorithm.zlib]
        if compression_algo_impls['brotli_compress']:
            algos.append(CompressionAlgorithm.brotli)
        if compression_algo_impls['zstd_compress']:
            algos.append(CompressionAlgorithm.zstd)
        ...
        for algo in algos:
            ...
            self.assertEqual(cc.write(),
                bytearray(b'\x19' +  # type of message - compressed certificate
                        int.to_bytes(message_len, 3, 'big') +
                        int.to_bytes(algo, 2, 'big') +
                        b'\x00\x00\x03' +  # length of uncompressed message
                        int.to_bytes(compressed_len, 3, 'big') +
                        compressed_cert))
    ...
    def test_parse_empty(self):
        ...
        for algo in algos:
            ...
            parser = Parser(
                bytearray(# b'\x19' +  # type of compressed certificate
                        int.to_bytes(message_len, 3, 'big') +
                        int.to_bytes(algo, 2, 'big') +
                        b'\x00\x00\x03' +  # length of uncompressed message
                        int.to_bytes(compressed_len, 3, 'big') +
    ...
    def test_parse_empty_with_wrong_expected_size(self):
        ...
        cc.compression_algo = CompressionAlgorithm.zlib
        compressed_cert = cc._compress(
            bytearray(b'\x00\x00\x00')  # length of the list of certs
        )
        ...
        parser = Parser(
            bytearray(# b'\x19' +  # type of compressed certificate
                    int.to_bytes(message_len, 3, 'big') +
                    int.to_bytes(CompressionAlgorithm.zlib, 2, 'big') +
                    b'\x00\x00\x04' +  # length of uncompressed message
                    int.to_bytes(compressed_len, 3, 'big') +
                    compressed_cert))
    ...
    def test_custom_decompression(self):
        cc = CompressedCertificate(CertificateType.x509)
        cc.compression_algo = CompressionAlgorithm.brotli
        msg = bytearray(100)
